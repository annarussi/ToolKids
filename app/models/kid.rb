...
class Kid < ApplicationRecord
  ...
  after_create :game_colors, :game_animals, :game_objects, :game_verbs

  def game_colors
    game_colors = Game.new(name: "Colors", level: 1, kid: self)
    game_colors.save
  ...
  def game_animals
    game_animals = Game.new(name: "Animals", level: 2, kid: self)
    game_animals.save
  ...
  def game_objects
    game_objects = Game.new(name: "Objects", level: 3, kid: self)
    game_objects.save
  end

  def game_verbs
    game_verbs = Game.new(name: "Verbs", level: 4, kid: self)
    game_verbs.save
  end
end
